...
if(isset($_POST['update_button']))
{
	...
	if($_SESSION['password'] == $_POST['update_password']){
		if(isset($new_update_password) && isset($update_comf_new_password) && empty($update_username)){
			if(!$uppercase || !$lowercase || !$number || !$character || strlen($new_update_password) < 8){
				header("Location: ./Update-profile.php?error=wrongpassword");
				exit();
			}
			else if($new_update_password!== $update_comf_new_password){
				header("Location: ./Update-profile.php?error=passwordsdontmatch");
				exit();
			}
			else
			{
				...
				{
					header("Location: ./Update-profile.php?update=success");
					exit();
				}
				else
				{
		...
		else if(empty($new_update_password) && empty($update_comf_new_password) && isset($update_username))
		{
			...
			{
				header("Location: ./Update-profile.php?error=usernametaken");
				exit();
			}
			else
			{
	...
	{
		header("Location: ./Update-profile.php?error=wrongcurrentpassword");
		exit();
	}
}
else {
	header("Location: ./Update-profile.php?error=unauthorisedaccess");
	exit();
}
?>
